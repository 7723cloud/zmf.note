...
class MainWindow(QMainWindow):
    ...
    def Init_UI(self):
        self.setWindowTitle("邹明甫.txt - 记事本")
        self.resize(1000, 800)
        self.centralwidget = QtWidgets.QWidget(self)
        self.setCentralWidget(self.centralwidget)

        self.openFileName = '邹明甫.txt'
        self.openFilePath = ''
        self.isSaved = False
        self.fintText = ''
        self.replaceText = ''
    ...
    def Copyright_UI(self):
        ...
        self.labelcopyright.setText("本软件版权所有归于邹明甫")
        formLayout.addRow(self.labelcopyright)

        self.labelcopyrightEng = QtWidgets.QLabel(self.copyrightDialog)
        self.labelcopyrightEng.setText("This software all copyright© from Minngfu Zou️")
    ...
    def saveFile(self):
        if self.openFilePath == "":
            filename, filetype = QtWidgets.QFileDialog.getSaveFileName(self, '保存', './',
                                                                       "文本文档 (*.txt);;所有文件 (*)")
            if filename == "":
                return False
